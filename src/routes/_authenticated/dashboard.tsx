...
import { Package, Users, FileText, TrendingUp, AlertTriangle, ArrowUpRight, Loader2 } from "lucide-react";
...
const paymentStatusLabel = (s: string) => s === "paid" ? "Payée" : s === "partial" ? "Partielle" : "Non payée";
const paymentStatusTone = (s: string): "success" | "warning" | "destructive" =>
  s === "paid" ? "success" : s === "partial" ? "warning" : "destructive";

function DashboardPage() {
  const { data: statsData } = useQuery({ queryKey: ["dashboard-stats"], queryFn: () => dashboardService.stats() });
  const { data: chartData } = useQuery({ queryKey: ["dashboard-chart"], queryFn: () => dashboardService.salesChart() });
  const { data: lowStockData } = useQuery({ queryKey: ["dashboard-low-stock"], queryFn: () => dashboardService.lowStock() });
  const { data: recentData } = useQuery({ queryKey: ["dashboard-recent"], queryFn: () => dashboardService.recentInvoices() });
  ...
  const statCards = [
    { label: "Total produits", value: stats?.totalProducts ?? "—", icon: Package, tint: "bg-primary-soft text-primary" },
    { label: "Total clients", value: stats?.totalCustomers ?? "—", icon: Users, tint: "bg-success-soft text-success" },
    { label: "Total factures", value: stats?.totalInvoices ?? "—", icon: FileText, tint: "bg-warning-soft text-warning" },
    { label: "Chiffre d'affaires", value: stats ? formatCurrency(stats.revenue) : "—", icon: TrendingUp, tint: "bg-accent/15 text-accent" },
    { label: "Stock faible", value: stats?.lowStock ?? "—", icon: AlertTriangle, tint: "bg-destructive-soft text-destructive" },
  ];
    ...
    <div className="space-y-6">
          ...
          return (
            <Card key={s.label} className="p-5">
              ...
              <div className="text-2xl font-bold text-foreground">
                {stats === undefined ? <Loader2 className="w-5 h-5 animate-spin text-muted-foreground" /> : s.value}
              </div>
              {s.label === "Stock faible" && stats && stats.lowStock > 0 && (
                <div className="flex items-center gap-1 text-xs font-medium mt-2 text-destructive">
                  <ArrowUpRight className="w-3 h-3" /> Critique
                </div>
              )}
            </Card>
          );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            {salesChart.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={salesChart} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 12, fill: "oklch(0.52 0.03 250)" }} axisLine={false} tickLine={false} tickFormatter={(v) => `${(v / 1000000).toFixed(1)}M`} />
        ...
        <Card className="p-5">
          ...
          {lowStock.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-8">Aucune alerte stock</p>
          ) : (
            <div className="space-y-3">
              {lowStock.slice(0, 5).map((p: { _id: string; name: string; category: string; quantity: number; alertThreshold: number }) => (
                <div key={p._id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-muted-foreground">Seuil {p.alertThreshold}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {recentInvoices.map((inv: { _id: string; invoiceNumber: string; customer: { fullName: string }; createdAt: string; totalAmount: number; paymentStatus: string }) => (
                  <tr key={inv._id} className="hover:bg-muted/30">
                    ...
                    <td className="px-5 py-3 hidden sm:table-cell text-muted-foreground">{formatDate(inv.createdAt)}</td>
                    <td className="px-5 py-3 text-right font-medium">{formatCurrency(inv.totalAmount)}</td>
                    <td className="px-5 py-3"><Badge tone={paymentStatusTone(inv.paymentStatus)}>{paymentStatusLabel(inv.paymentStatus)}</Badge></td>
                  </tr>
                ))}
              </tbody>
            </table>
